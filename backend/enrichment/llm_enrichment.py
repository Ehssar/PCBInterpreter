...
def _sanitize_resolved_type(value: Any) -> str | None:
    allowed = {
...
def _normalize_enrichment_payload(payload: dict[str, Any]) -> dict[str, Any]:
    return {
        "resolved_type": _sanitize_resolved_type(payload.get("resolved_type")),
        "display_name": payload.get("display_name"),
        "one_line_label": payload.get("one_line_label"),
        "function_summary": payload.get("function_summary"),
        "confidence_note": payload.get("confidence_note"),
...
# Potentially Useful for a more robust system
...
def enrich_component(
    ...
    developer_prompt = (
        "Enrich PCB component detections for a small AR label.\n"
